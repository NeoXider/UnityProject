#ifndef TEXTURE_ADJUSTMENTS_MATH_CGINC
#define TEXTURE_ADJUSTMENTS_MATH_CGINC 
...
#define NORMALIZE_NOISE_TEXTURE_UV (_MainTex_TexelSize.zw / 512.0)
...
float4 Sobel(sampler2D tex, float2 uv, float2 delta)
{
	float4 x = 0;
	float4 y = 0;


	x += tex2D(tex, uv + float2(-1, -1) * delta) * -1.0;
	x += tex2D(tex, uv + float2(-1,  0) * delta) * -2.0;
	x += tex2D(tex, uv + float2(-1,  1) * delta) * -1.0;
	...
	y += tex2D(tex, uv + float2(-1, -1) * delta) * -1.0;
	y += tex2D(tex, uv + float2( 0, -1) * delta) * -2.0;
	y += tex2D(tex, uv + float2( 1, -1) * delta) * -1.0;
